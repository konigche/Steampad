 ...
 * (A7-item). Pressing it toggles and notifies the listener. Built on {@link PressableWidget} so it
 * works with mouse, keyboard, and the controller's focus-activate path.
 */
public class SteamToggle extends PressableWidget {

    private boolean on;
    private final Text label;
    private final Consumer<Boolean> onChange;

    public SteamToggle(int x, int y, int w, int h, Text label, boolean on, Consumer<Boolean> onChange) {
        super(x, y, w, h, label);
        this.on = on;
        this.label = label;
    ...
    protected void renderWidget(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.drawText(MinecraftClient.getInstance().textRenderer, label, x + 6, y + (h - 8) / 2,
                0xFFE6ECF2, false);
