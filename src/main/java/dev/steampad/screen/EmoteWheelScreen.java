...
public class EmoteWheelScreen extends SteamPadBaseScreen {
    ...
    private record Row(ButtonWidget button, String emoteId) {}
    ...
    public EmoteWheelScreen(Screen parent, long handle) {
        super(Text.translatable("steampad.emote.wheel.title"));
        this.parent = parent;
        this.handle = handle;
    ...
    private void rebuild() {
        ...
        cfg.ensureEmoteWheel();   // guarantees at least one wheel exists
        cfg.normalize();
        ...
        // Wheel row — add/remove/switch, same shape as the regular radial editor (feedback: "el
        // funcionamiento debe ser similar [al] radial menu, con sus glifos de cómo cambiar de rueda").
        int third = (w - 8) / 3;
        addDrawableChild(ButtonWidget.builder(Text.literal((page + 1) + "/" + wheels), b -> {})
                .dimensions(x, y, w, 14).build()).active = false;
        y += 14;
        ButtonWidget prevW = ButtonWidget.builder(Text.literal("◀"), b -> switchWheel(-1))
                .dimensions(x, y, third, 18)
        ...
        ButtonWidget addW = ButtonWidget.builder(Text.translatable("steampad.radial.wheel.add_short"), b -> addWheel())
                .dimensions(x + third + 4, y, third, 18)
                .tooltip(Tooltip.of(Text.translatable("steampad.radial.wheel.add"))).build();
        addW.active = wheels < RadialConfig.MAX_WHEELS;
        addScroll(addW, y);
        ButtonWidget delW = ButtonWidget.builder(Text.translatable("steampad.radial.wheel.remove_short"), b -> removeWheel())
                .dimensions(x + (third + 4) * 2, y, third, 18)
        ...
        }).dimensions(x, y, third, 20).build();
        ButtonWidget count = ButtonWidget.builder(
                Text.translatable("steampad.emote.wheel.slots", slotCount), b -> {})
                .dimensions(x + third + 4, y, third, 20).build();
        count.active = false;
        ...
        }).dimensions(x + (third + 4) * 2, y, third, 20).build();
        addScroll(minus, y);
        addScroll(count, y);
        addScroll(plus, y);
        y += ROW_H + 4;
        ...
        for (int i = 0; i < slotCount; i++) {
            ...
            Text label = isEmote
                    ? Text.literal((slotIdx + 1) + ". " + displayNameFor(sc))
                    : Text.translatable("steampad.emote.wheel.slot_empty", slotIdx + 1);
            ButtonWidget row = ButtonWidget.builder(label, b -> {
                UiSoundService.playSelect();
                client.setScreen(new EmoteLibraryScreen(this, emoteId -> assign(slotIdx, emoteId)));
            }).dimensions(x, y, w - clearW - 4, 20).build();
            addScroll(row, y);
            rows.add(new Row(row, isEmote ? sc.action : null));
            ButtonWidget clear = ButtonWidget.builder(Text.literal("✖"), b -> {
                clearSlot(slotIdx);
                rebuild();
            }).dimensions(x + w - clearW, y, clearW, 20).build();
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        EmotePreviewPanel.render(ctx, textRenderer, px, py, PREVIEW_W,
                this.height - FOOTER_H - py - 8, previewData,
                showEntity ? ENTITY_INSET : 0, previewData != null && !hasPlayer);
        if (showEntity) {
            int ex1 = px + PREVIEW_W / 2 - 24, ex2 = px + PREVIEW_W / 2 + 24;
            InventoryScreen.drawEntity(ctx, ex1, py + 4, ex2, py + ENTITY_INSET,
                    26, 0f, 0f, delta, MinecraftClient.getInstance().player);
        }
    }
